...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.AppName = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["appType"]
	if f0ok {
		r.ko.Spec.AppType = &f0
	}
	f1, f1ok := identifier.AdditionalKeys["domainID"]
	if f1ok {
		r.ko.Spec.DomainID = &f1
	}
	f2, f2ok := identifier.AdditionalKeys["userProfileName"]
	if f2ok {
		r.ko.Spec.UserProfileName = &f2
	}
